...
databinding:使用两种数据方式:一种是LiveData+ViewModel,一种是Observable:Observable又分为两种:一种是 extends ViewModel implements Observable,还有一种是 implements Observable;

This sample showcases the following features of the
[Data Binding library](https://developer.android.com/topic/libraries/data-binding/index.html):

* Layout variables and expressions 布局变量和表达式
* Observability through Observable Fields, LiveData and Observable classes 通过Observable Fields,LiveData,Observable classes将数据变成可观察变量(就是变量发生变化后,UI自动改变)
* Binding Adapters, Binding Methods and Binding Converters 绑定适配器(自定义布局属性,通过函数修改布局内容),绑定方法(自定义布局属性,或者是已有属性,将属性和相对应的函数关联起来),绑定转化器(将已有属性的值类型转换成其他类型,同时在函数中对属性进行赋值)
* Seamless integration with ViewModels 与ViewModel进行无缝集成
...
features:
  ...
  * 
  ...
  ViewModel activity: Using a ViewModel from the fixes the rotation problem and moves logic out of the activity. Also, the use of binding
